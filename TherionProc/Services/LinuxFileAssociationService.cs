...
// use it writes a desktop entry (~/.local/share/applications/therionproc.desktop) plus a custom
...
{
    private const string DesktopFile = "therionproc.desktop";
    ...
    public FileAssociationInfo GetInfo(string extension)
    {
        ...
        try
        {
            ...
            if (string.Equals(def, DesktopFile, StringComparison.OrdinalIgnoreCase))
                return new(ext, desc, FileAssociationState.Associated, "TherionProc");
    ...
    private void EnsureDesktopAndMime()
    {
        ...
        File.WriteAllText(Path.Combine(_mimeDir, "packages", "therionproc.xml"), mime.ToString());

        var desktop = new StringBuilder();
        desktop.AppendLine("[Desktop Entry]");
        desktop.AppendLine("Type=Application");
        desktop.AppendLine("Name=TherionProc");
