...
//
// Used by the Overview ▸ Audit "orphan files" scan to decide which .th/.th2 files on disk are truly
// orphans — bound to *no* thconfig in the workspace, not merely absent from the *active* entry
// point's graph. Answering that means walking the source graph of every thconfig in the directory,
// which is why the audit only runs on demand (see ProjectAuditViewModel). This helper does the heavy
// walk without watchers, disk cache or semantic binding, so it is cheap enough to run off the UI
// thread. It intentionally mirrors ProjectEntryDiscovery's BFS (same SourceGraph edges) so the two
...
public static class WorkspaceReachability
{
    ...
    public static HashSet<string> ReachableFrom(IEnumerable<string> entryPoints, CancellationToken ct = default)
    {
        ...
        while (queue.Count > 0 && budget-- > 0)
        {
            ...
            TherionFile? file;
            try { file = DocumentParser.ParseByExtension(path, EncodingResolver.ReadAllText(path)).Value; }
            catch { continue; }
