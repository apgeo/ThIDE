...
- **Open…** loads a `.lox`/`.3d` model file directly; after a build, the newest model auto-loads
  (`.lox` preferred over `.3d`) when auto-show is on. A model older than its sources is flagged
  **stale** (rebuild to refresh — BUILD-03).
- **Color by:** Height · Survey · Length · Inclination · Plain (the last choice is remembered).
- **Reset view** re-frames the model; **Refresh** reloads it.
...
- **Show in 3D View** — right-click a station (or a shot's *From*/*To*) in the Measurements grid to
  select and frame it in the 3D view.
