...
public partial class TherionTextEditor : UserControl
{
    ...
    public static readonly Avalonia.Input.DataFormat<string> InsertPathFormat =
        Avalonia.Input.DataFormat.CreateInProcessFormat<string>("thide-insert-path");
    ...
    private MenuItem? _encloseRegionMenuItem;            // "Enclose in region" (Therion files; enabled for a multi-line block)
    ...
    private void ConfigureEditor(TextEditor editor)
    {
        ...
        ThIDE.Resources.LocProxy.Instance.PropertyChanged += OnUiLanguageChanged;
        DetachedFromVisualTree += (_, _) =>
            ThIDE.Resources.LocProxy.Instance.PropertyChanged -= OnUiLanguageChanged;
    ...
    private void OnEditorContextRequested(object? sender, ContextRequestedEventArgs e)
    {
        ...
        if (_encloseRegionMenuItem is not null)
        {
            _encloseRegionMenuItem.IsVisible = IsTherionSourceFile;
            _encloseRegionMenuItem.IsEnabled = IsMultiLineSelection;   // active only for a block
        }
    ...
    private ContextMenu BuildContextMenu()
    {
        ...
        // "Enclose in region": shown for Therion files, enabled only for a multi-line selection
        // (gated per-open in OnEditorContextRequested).
        _encloseRegionMenuItem = MakeItem(L("Ed_EncloseRegion"), () => _ = EncloseInRegionAsync());
        menu.Items.Add(_encloseRegionMenuItem);
        menu.Items.Add(new Separator());
        // line operations.
    ...
    /// <summary>True when the current file is a Therion source file (.th/.th2/.thconfig/.thc).</summary>
    private bool IsTherionSourceFile
    {
        get
        {
            var ext = System.IO.Path.GetExtension(CurrentFilePath ?? string.Empty).ToLowerInvariant();
            return ext is ".th" or ".th2" or ".thconfig" or ".thc";
        }
    }

    /// <summary>True when the selection spans more than one line (a block).</summary>
    private bool IsMultiLineSelection
    {
        get
        {
            if (_editor is null || _editor.SelectionLength <= 0) return false;
            var doc = _editor.Document;
            return doc.GetLineByOffset(_editor.SelectionStart).LineNumber
                != doc.GetLineByOffset(_editor.SelectionStart + _editor.SelectionLength).LineNumber;
        }
    }

    /// <summary>
    /// Menu/palette/keybinding entry point for "Enclose in region" (fire-and-forget). Wraps the
    /// selected lines in a <c>#@region '&lt;title&gt;'</c> … <c>#@endregion</c> block after prompting
    /// for the title. No-op outside Therion source files.
    /// </summary>
    public void MenuEncloseInRegion() => _ = EncloseInRegionAsync();

    /// <summary>
    /// Prompts for a region title, then encloses the selected lines (or the caret line) in a
    /// <c>#@region</c>/<c>#@endregion</c> pair. The directives take whole lines and copy the
    /// starting line's indentation, so any partial selection still wraps the full first/last line.
    /// </summary>
    public async System.Threading.Tasks.Task EncloseInRegionAsync()
    {
        if (_editor is null || !IsTherionSourceFile) return;
        if (TopLevel.GetTopLevel(this) is not Window owner) return;

        var (first, last) = SelectedLineRange();

        var title = await new ThIDE.Views.InputDialog(
            L("Ed_EncloseRegion"), L("Region_TitlePrompt"), string.Empty).ShowAsync(owner);
        if (title is null) return; // cancelled

        var doc = _editor.Document;
        var indent = LeadingWhitespace(doc.GetText(first));
        var nl = NewlineFor(doc);

        doc.BeginUpdate();
        try
        {
            // Insert the closer first (higher offset) so the opener insert keeps first.Offset valid.
            doc.Insert(last.EndOffset, nl + indent + Therion.Syntax.Directives.RegionDirective.EndLine());
            doc.Insert(first.Offset,
                indent + Therion.Syntax.Directives.RegionDirective.StartLine(title) + nl);
        }
        finally
        {
            doc.EndUpdate();
    ...
    private static string L(string key) => ThIDE.Resources.Tr.Get(key);
