...
public partial class MainWindowViewModel : ViewModelBase
{
    ...
    public WelcomeToolViewModel WelcomeTool { get; }
    ...
    {
        var docs = ThIDE.AppServices.Provider.GetService(typeof(IThbookDocumentationService))
    ...
    public string MenuEditEncloseRegion => L("Menu_Edit_EncloseRegion", "Enclose in Region");
    ...
    private static readonly string[] LocalizedMenuProps =
    {
        ...
        nameof(MenuEditToggleComment), nameof(MenuEditEncloseRegion), nameof(MenuEditFoldAll), nameof(MenuEditUnfoldAll),
    ...
    public MainWindowViewModel(
        ...
        DockFactory factory,
        WelcomeToolViewModel welcomeTool,
        ...
        IScriptHookService? hooks = null)
    {
        ...
        WelcomeTool.Welcome.OpenFileRequested += (_, _) => OpenFileCommand.Execute(null);
        WelcomeTool.Welcome.OpenFolderRequested += (_, _) => OpenFolderCommand.Execute(null);
        WelcomeTool.Welcome.ShowExternalToolsSettingsRequested += (_, _) => ShowPreferencesRequested?.Invoke(this, "external");
    ...
    public MainWindowViewModel() : this(
        ...
        DesignFactory(),
        new WelcomeToolViewModel(new WelcomeViewModel()),
    ...
    private async Task RestoreSessionAsync(AppSettings? s)
    {
        ...
        // Show the Welcome start page when nothing was restored (and the option is on). Posted at
        // Background priority AFTER the placeholder/float restore above so the document count is final.
        Avalonia.Threading.Dispatcher.UIThread.Post(
            MaybeShowWelcomeOnStartup, Avalonia.Threading.DispatcherPriority.Background);

        // offer to recover unsaved buffers left by a crashed run.
        OfferCrashRecovery();
    }

    /// <summary>Opens the Welcome start page at launch when no files/workspace were restored (#).</summary>
    private void MaybeShowWelcomeOnStartup()
    {
        if (_settings?.Current.ShowWelcomeOnStartup != true) return;
        bool nothingRestored = _documents.Documents.Count == 0 && string.IsNullOrEmpty(_session?.RootPath);
        if (nothingRestored) _factory.ShowToolInDocuments(WelcomeTool);
    }
    ...
    {
        if (_picker is null) return;
        var path = await _picker.PickSaveFileAsync(Tr.Get("Welcome_NewFile"), "new.th").ConfigureAwait(true);
        if (string.IsNullOrEmpty(path)) return;
        try
        {
