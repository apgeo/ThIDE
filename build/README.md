...
Native application installers for TherionProc. Both wrap the same **self-contained, single-file**
...
|----------|--------|--------|---------|
| Windows  | `TherionProc-Setup-<ver>.exe` | [`windows/build-installer.ps1`](windows/build-installer.ps1) → [`windows/TherionProc.iss`](windows/TherionProc.iss) | [Inno Setup 6](https://jrsoftware.org/isdl.php) (`ISCC.exe`) |
| Linux    | `therionproc_<ver>_amd64.deb` | [`linux/build-deb.sh`](linux/build-deb.sh) | `dpkg-deb` (+ `icoutils`/ImageMagick for the icon) |
| Linux    | `TherionProc-<ver>-x86_64.AppImage` | [`linux/build-appimage.sh`](linux/build-appimage.sh) | `appimagetool` (auto-downloaded; + `icoutils`/ImageMagick) |
...
clean uninstall (Add/Remove Programs on Windows, `apt remove therionproc` on Linux); the **AppImage**
...
# -> build/windows/Output/TherionProc-Setup-0.3.0.exe
```
...
build/linux/build-deb.sh      publish/linux-x64 0.3.0   # -> ./therionproc_0.3.0_amd64.deb
build/linux/build-appimage.sh publish/linux-x64 0.3.0   # -> ./TherionProc-0.3.0-x86_64.AppImage
```
