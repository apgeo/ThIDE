...
public static class SourceFileIo
{
    ...
    public static void Write(SourceFile file, string text)
    {
        ...
        var payload = file.Encoding.GetBytes(text);
    ...
    /// <summary>Creates a new file as UTF-8 without a BOM — the encoding Therion assumes by default.</summary>
    public static void Create(string path, string content)
    {
        var directory = System.IO.Path.GetDirectoryName(path);
        if (!string.IsNullOrEmpty(directory)) Directory.CreateDirectory(directory);

        WriteAtomic(path, EncodingResolver.Default.GetBytes(content));
    }
