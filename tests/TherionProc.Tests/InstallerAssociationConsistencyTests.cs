...
/// there without updating the installers, these tests fail. See build/windows/TherionProc.iss,
/// build/linux/build-deb.sh and build/linux/build-appimage.sh.
/// </summary>
public class InstallerAssociationConsistencyTests
{
    // Walk up from the test binary until we find the repo root (the folder holding TherionProc.sln).
    private static string RepoRoot()
    {
        var dir = new DirectoryInfo(AppContext.BaseDirectory);
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "TherionProc.sln")))
            dir = dir.Parent;
        Assert.NotNull(dir);
        return dir!.FullName;
    }
    ...
    {
        var iss = ReadBuildFile("build", "windows", "TherionProc.iss");
        foreach (var (ext, _) in FileAssociationCatalog.Types)
        {
            ...
            Assert.True(iss.Contains($@"Software\Classes\{ext}""", StringComparison.Ordinal),
                $"TherionProc.iss is missing the extension key for '{ext}'.");
            // And its ProgId,  e.g.  TherionProc.th2
            Assert.True(iss.Contains($"TherionProc{ext}", StringComparison.Ordinal),
                $"TherionProc.iss is missing the ProgId 'TherionProc{ext}'.");
        }
    }
